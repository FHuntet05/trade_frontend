// frontend/src/components/tools/PurchaseModal.jsx (VERSIÓN v17.3 FINAL - CON NAVEGACIÓN)
...
const PurchaseModal = ({ tool, onClose, onInitiateCryptoPayment }) => {
  const { user, updateUser } = useUserStore();
  const [isProcessing, setIsProcessing] = useState(false);
  const [quantity, setQuantity] = useState(1);
  const MAX_QUANTITY = 20;

  const handleIncrease = () => { if (quantity < MAX_QUANTITY) setQuantity(q => q + 1); };
  ...
  const handlePayWithBalance = async () => {
    ...
    const purchasePromise = api.post('/wallet/purchase-with-balance', {
      toolId: tool._id,
      quantity: quantity,
    });
  ...
  const handlePayWithCrypto = () => {
    onInitiateCryptoPayment(tool, quantity, totalCost);
  };
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
        ...
        >
          ...
          <div className="space-y-2 text-sm my-4">
            <div className="flex justify-between items-center bg-dark-primary/50 p-3 rounded-lg"><span className="text-text-secondary">Costo Total</span><span className="font-bold text-xl text-white">{totalCost.toFixed(2)} USDT</span></div>
            <div className="flex justify-between items-center bg-dark-primary/50 p-3 rounded-lg"><span className="text-text-secondary">Tu Saldo</span><span className={`font-bold text-xl ${canPayWithBalance ? 'text-green-400' : 'text-red-400'}`}>{userBalance.toFixed(2)} USDT</span></div>
          </div>
          <div className="mt-6">
            <button 
              onClick={canPayWithBalance ? handlePayWithBalance : handlePayWithCrypto} 
              disabled={isProcessing}
              className="w-full flex items-center justify-center gap-3 p-3 rounded-full bg-gradient-to-r from-accent-start to-accent-end text-white font-bold disabled:bg-gray-600 disabled:opacity-50 transition-all shadow-glow"
            >
            ...
            {!canPayWithBalance && <p className="text-xs text-center text-text-secondary mt-2">Se te guiará para realizar el pago con criptomonedas.</p>}
          </div>
      </motion.div>
    </div>
  );
