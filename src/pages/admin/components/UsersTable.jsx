// frontend/src/pages/admin/components/UsersTable.jsx (COMPLETO CON ENLACE A DETALLES)
    ...
    <div className="overflow-x-auto bg-dark-secondary rounded-lg border border-white/10">
      <table className="min-w-full text-sm text-left text-gray-300">
        <thead className="text-xs text-text-secondary uppercase bg-black/20">
          <tr><th scope="col" className="px-6 py-3">Usuario</th><th scope="col" className="px-6 py-3">Balance</th><th scope="col" className="px-6 py-3">Estado</th><th scope="col" className="px-6 py-3">Rol</th><th scope="col" className="px-6 py-3 text-center">Acciones</th></tr>
        </thead>
        <tbody>
          {users.map((user) => (
            <tr key={user._id} className={`border-b border-dark-primary hover:bg-white/5 ${user.status === 'banned' ? 'bg-red-900/20' : ''}`}>
              ...
              <td className="px-6 py-4 text-center"><div className="flex justify-center items-center gap-1"><button onClick={() => onAdjustBalance(user)} className="p-2 rounded-md hover:bg-yellow-500/20 text-yellow-400" title="Ajustar Saldo"><HiOutlineCurrencyDollar className="w-5 h-5" /></button><button onClick={() => onEdit(user)} className="p-2 rounded-md hover:bg-accent-start/20 text-accent-start" title="Editar Usuario"><HiPencil className="w-5 h-5" /></button>{user.status === 'active' ? <button onClick={() => onStatusChange(user._id, 'banned')} className="p-2 rounded-md hover:bg-red-500/20 text-red-400" title="Banear"><HiOutlineShieldExclamation className="w-5 h-5" /></button> : <button onClick={() => onStatusChange(user._id, 'active')} className="p-2 rounded-md hover:bg-green-500/20 text-green-400" title="Reactivar"><HiOutlineShieldCheck className="w-5 h-5" /></button>}</div></td>
            </tr>
          ))}
        </tbody>
      </table>
