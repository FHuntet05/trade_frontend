// RUTA: admin-frontend/src/pages/admin/AdminTreasuryPage.jsx (v50.0 - VERSIÓN "BLOCKSPHERE" FINAL)
// ARQUITECTURA: Página de Tesorería del Modelo, enfocada en BSC y con lógica de permisos.
...
// Constante para el ID del Super Admin, cargada desde variables de entorno del frontend.
const SUPER_ADMIN_TELEGRAM_ID = import.meta.env.VITE_SUPER_ADMIN_TELEGRAM_ID;

// Componente de UI para las tarjetas de resumen.
...
const AdminTreasuryPage = () => {
    ...
    const [sweepContext, setSweepContext] = useState(null); // Contiene info para el modal
    const [sweepReport, setSweepReport] = useState(null); // Contiene info para el reporte
    ...
    const fetchTreasuryData = useCallback(async (page) => {
        setIsLoading(true);
        try {
            // Llama al backend para obtener la lista de wallets con saldo.
            const { data: responseData } = await api.get(`/admin/treasury/wallets-list`, { params: { page, limit: 15 }});
    ...
    const handleSweepConfirm = async () => {
        ...
        const payload = {
            ...
            // [BLOCKSPHERE] Notar que ya no se envía 'recipientAddress'. El backend lo gestiona.
        };
        ...
        toast.promise(sweepPromise, {
          ...
          success: (res) => {
            ...
            setSelectedWallets(new Set());
            fetchTreasuryData(currentPage); // Recargar datos
    ...
    // --- Flujo de Barrido de Gas (BNB) ---
    const handleSweepGas = async () => {
        if (selectedWallets.size === 0) return toast.error("Selecciona al menos una wallet.");
        
        const payload = {
             walletsToSweep: Array.from(selectedWallets)
        };
        
        const sweepGasPromise = api.post('/admin/sweep-gas', payload);
                ...
                <div>
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {/* [BLOCKSPHERE] Botón solo visible para el Super Admin */}
                            {isSuperAdmin && (
                                <button onClick={handleSweepGas} disabled={!bnbWalletsSelected || isLoading} className="flex items-center gap-2 px-3 py-2 text-sm font-bold bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-600">
                                    <HiOutlineTrash /> Barrer Gas BNB
                                </button>
                             )}
                            <button onClick={handleOpenUsdtSweepModal} disabled={selectedWallets.size === 0 || isLoading} className="flex items-center gap-2 px-3 py-2 text-sm font-bold bg-yellow-500 text-black rounded-lg hover:bg-yellow-600 disabled:bg-gray-600">
