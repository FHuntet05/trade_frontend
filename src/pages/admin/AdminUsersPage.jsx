// RUTA: frontend/src/pages/admin/AdminUsersPage.jsx (VERSIÓN "NEXUS - ENHANCED UI")
...
// Componente para la celda de acciones, ahora más limpio y desacoplado.
            ...
            <div className="flex justify-center items-center gap-2">
                ...
                {isSuperAdmin && user.role === 'admin' && (
                    <>
                        ...
                        {user.status === 'active' ? (
                            <button onClick={() => onAction('toggleBan', user)} title="Banear Administrador" className="p-2 text-red-400 hover:bg-red-500/20 rounded-full transition-colors">
                                <HiOutlineNoSymbol className="w-5 h-5" />
                            </button>
                        ) : (
                            <button onClick={() => onAction('toggleBan', user)} title="Desbanear Administrador" className="p-2 text-green-400 hover:bg-green-500/20 rounded-full transition-colors">
...
const AdminUsersPage = () => {
    ...
    const handlePromoteSubmit = async (userId, password) => {
        ...
        toast.promise(promise, {
            ...
            success: () => { setModalType(null); fetchUsers(); return 'Usuario promovido a administrador.'; },
    ...
    const handleToggleBan = (user) => {
        ...
        if (window.confirm(`¿Seguro que quieres ${actionText} a ${user.username}?`)) {
            // [NEXUS ENHANCED UI - CORRECCIÓN CRÍTICA]
            // Se utiliza el endpoint PUT /admin/users/:id para cambiar CUALQUIER propiedad del usuario.
            ...
            toast.promise(promise, {
                ...
                success: () => { fetchUsers(); return `Usuario ${actionText === 'banear' ? 'baneado' : 'reactivado'}.`; },
    ...
    const handleResetPasswordSubmit = async (adminId) => {
        ...
        toast.promise(promise, {
            ...
            success: (res) => { setModalType(null); window.prompt('Copia la nueva contraseña temporal:', res.data.temporaryPassword); return 'Contraseña reseteada.'; },
