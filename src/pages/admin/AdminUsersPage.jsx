// RUTA: frontend/src/pages/admin/AdminUsersPage.jsx (VERSIÓN CON REGLAS DE ACCESO ACTUALIZADAS)
...
// ✅ Acciones con reglas de superadmin y admins
const ActionsCell = ({ user, onAction }) => {
    ...
    // 1. Superadmin: su propia fila queda bloqueada
    if (user.telegramId?.toString() === superAdminIdFromEnv?.trim()) {
        return (
            <td className="p-3 text-center">
                <span className="text-gray-500 text-sm italic">Protegido</span>
            </td>
        );
    }

    // 2. Admin normal: no puede tocar admins ni superadmin
    if (!isSuperAdmin && user.role === 'admin') {
        return (
            <td className="p-3 text-center">
                <span className="text-gray-500 text-sm italic">Restringido</span>
            </td>
        );
    }

    // 3. Superadmin puede gestionar a todos los demás
    // 4. Admin normal puede gestionar solo usuarios normales
    return (
            ...
            <div className="flex justify-center items-center gap-2">
                ...
                {(isSuperAdmin || user.role !== 'admin') && (
                    <>
                        ...
                        {user.status === 'active' ? (
                            <button
                                onClick={() => onAction('toggleBan', user)}
                ...
                <div className="bg-dark-secondary p-4 rounded-lg border border-white/10">
                    <form onSubmit={handleSearch} className="flex items-center gap-2">
                        <input
                            ...
                            className="w-full bg-dark-primary p-2 rounded-md border border-white/10"
                        />
                                        ...
                                        <tr key={user._id} className="hover:bg-dark-tertiary/50 transition-colors">
                                            <td className="p-3 font-medium">
                                                <Link
                                                    ...
                                                    className="flex items-center gap-3 hover:text-accent-start"
                                                >
                                                    <img
                                                        src={user.photoUrl}
                                                        alt="avatar"
