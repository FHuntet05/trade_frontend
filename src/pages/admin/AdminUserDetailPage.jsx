...
const TransactionsTable = ({ userId, initialTransactions }) => {
    ...
    return (
        <div className="bg-dark-secondary p-6 rounded-lg border border-white/10">
            ...
            {!isLoading && txItems.length === 0 ? <p className="text-text-secondary text-center py-4">No hay transacciones.</p> : (
                <>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="text-xs text-text-secondary uppercase bg-dark-tertiary">
                                <tr>
                                    <th className="p-3">Fecha</th><th className="p-3">Tipo</th><th className="p-3">Monto</th><th className="p-3">Descripción</th>
                                </tr>
                            </thead>
                            <tbody>
                                {txItems.map(tx => (
